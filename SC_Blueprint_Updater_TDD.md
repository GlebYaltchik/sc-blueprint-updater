...
### 6.8 Защита от частичного совпадения в именах BP

**Проблема:** Если BP `AAA` есть в списке, а `AAA BBB` — другой (не owned) предмет
в `global.ini`, `re.escape("AAA")` матчит `AAA` внутри строки `AAA BBB`.

**Решение:** Добавить к каждому паттерну отрицательный lookahead `(?!\s[^a-z\s])`.
Не совпадает, если после имени BP стоит пробел + не-строчная/не-пробельная литера
(заглавная, цифра, `'`, `"`, `&`, `(` и т.д.). SC-имена — Title Case, описательный
текст начинается со строчной.

| Значение              | После BP  | Результат     |
|-----------------------|-----------|---------------|
| `AAA BBB`             | ` B` fail | не матчит ✓   |
| `AAA 'Special'`       | ` '` fail | не матчит ✓   |
| `AAA & Werner`        | ` &` fail | не матчит ✓   |
| `the AAA blueprint`   | ` b` ok   | матчит ✓      |
| `received AAA.`       | `.` ok    | матчит ✓      |

**Ограничение:** если описательный текст содержит заглавное слово сразу после
имени BP (`"the AAA Blueprint of Y"`), совпадения не будет. На практике
в `global.ini` такое крайне редко.
